				...
				<view class="section">
					...
					<view v-else class="plans-list">
						<view 
							...
							:key="plan.id" 
							class="plan-card"
							@click="startRecitation(plan)"
							@longpress="showPlanMenu(plan)"
						>
							<view class="plan-header">
								<text class="plan-name">{{ plan.name }}</text>
								<view class="plan-status" :class="{ 'status-active': isPlanActive(plan) }">
									<text class="status-text">{{ isPlanActive(plan) ? '进行中' : '未开始' }}</text>
								</view>
							</view>
							...
							<view class="plan-details">
								...
								<view class="detail-item">
									...
									<text class="detail-value">{{ formatDate(plan.createTime) }}</text>
								</view>
							</view>
...
const getPlanProgress = (plan: Plan) => {
	...
	return Math.round((completedCount / plan.items.length) * 100);
};
